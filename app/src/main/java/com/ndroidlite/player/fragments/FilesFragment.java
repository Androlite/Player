...
import com.ndroidlite.player.misc.DialogAsyncTask;
import com.ndroidlite.player.misc.UpdateToastMediaScannerCompletionListener;
import com.ndroidlite.player.misc.WrappedAsyncTaskLoader;
import com.ndroidlite.player.model.Song;
import com.ndroidlite.player.utils.FileUtil;
...
import static com.ndroidlite.player.R.id.container;

/**
 * A simple {@link Fragment} subclass.
 */
public class FilesFragment extends AdlMainActivityFragment implements MainActivity.MainActivityFragmentCallbacks, CabHolder, BreadCrumbLayout.SelectionCallback, SongFileAdapter.Callbacks, AppBarLayout.OnOffsetChangedListener, LoaderManager.LoaderCallbacks<List<File>> {

    public static final String TAG = FilesFragment.class.getSimpleName();

    private static final int LOADER_ID = LoaderIds.FILE_FRAGMENT;
    ...
    private static class AsyncFileLoader extends WrappedAsyncTaskLoader<List<File>> {
        ...
        public AsyncFileLoader(FilesFragment filesFragment) {
            super(filesFragment.getActivity());
            fragmentWeakReference = new WeakReference<>(filesFragment);
        }

        @Override
        public List<File> loadInBackground() {
            FilesFragment filesFragment = fragmentWeakReference.get();
            File directory = null;
            if (filesFragment != null) {
                BreadCrumbLayout.Crumb crumb = filesFragment.getActiveCrumb();
                if (crumb != null) {
                    directory = crumb.getFile();
                }
            }
            if (directory != null) {
                List<File> files = FileUtil.listFiles(directory, filesFragment.getFileFilter());
                Collections.sort(files, filesFragment.getFileComparator());
                return files;
            } else {
